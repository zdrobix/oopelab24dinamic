...
void introducere_date ( int* zi, float* suma, char* tip) {
    ///utilizatorul introducele datele unei cheltuieli
    int zi_noua; float suma_noua; char* tip_nou = nullptr;
    ...
    fgets(tip_nou, sizeof(tip_nou), stdin);

    tip_nou[strcspn(tip_nou, "\n")] = '\0';
...
void meniu_filtrare (ListaCheltuieli* list) {
    ...
    **/
    ...
    int zi = -1; float suma = -1; char *tip = nullptr; int ok_optiune = 1;
    ...
    } else { printf("Optiune invalida!"); ok_optiune = 0;}
...
void meniu_ordonare(ListaCheltuieli* list) {
    ...
    **/
    ...
    ListaCheltuieli lista_ordonata = *list;

    if (optiune == '1' && ok_optiune) {

        printf("Lista ordonata dupa zi este: \n");

        sorteazaCh(&lista_ordonata, directie, 1, 0);

    } else if (optiune == '2' && ok_optiune) {
